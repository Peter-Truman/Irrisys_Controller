...
- **R3 — No/minimal blocking code.** Where blocking is unavoidable, run it as a **state machine**, never a busy-wait. **Threshold (user, 2026-07-13): blocking under ~10 ms is acceptable** — it has no practical impact on performance, so don't spend effort removing it. Only operations that block ≥10 ms need de-blocking. Current violators: LCD serial TX (~13.5 ms/line, ~74 ms for a full 4-line flush) and EEPROM block writes (~512 ms/block). Beeps were fixed in 2.5a; `__delay_us` ADC/pacing and the 5 ms LCD inter-frame delay are all under the threshold and can stay.
